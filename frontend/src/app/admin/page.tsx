    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
          ...
          <div className="text-3xl font-black text-emerald-600">{datasets.length}</div>
          <div className="text-xs text-slate-500 mt-2">Dataset wilayah aktif</div>
        </div>
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Total Masjid Tersimpan</div>
          <div className="text-3xl font-black text-emerald-600">{totalMasjids}</div>
          <div className="text-xs text-slate-500 mt-2">Masjid terproses ML & Graph</div>
        </div>
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Status Pipeline</div>
          <div className="text-3xl font-black text-teal-600">Aktif</div>
          <div className="text-xs text-slate-500 mt-2">ML Enrichment Online</div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <TabsList className="bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm">
            <TabsTrigger
              value="datasets"
        ...
        <TabsContent value="benchmarks" className="outline-none">
          <div className="grid grid-cols-1 gap-8">
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl mb-2 text-sm text-amber-900">
